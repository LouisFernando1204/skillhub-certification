        ...
        <form action="{{ route('courses.store') }}" method="POST">
            ...
            <div class="mb-6">
                ...
                <textarea name="description" id="description"
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('description') border-pink-500 @enderror">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-pink-500 text-xs italic">{{ $message }}</p>
                @enderror
            </div>
