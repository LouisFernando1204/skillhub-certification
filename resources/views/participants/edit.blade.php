...
@section('content')
    <div class="max-w-2xl mx-auto bg-white shadow p-6 rounded-lg">
        <h2 class="text-2xl font-bold mb-6">Edit Data Peserta</h2>
        ...
        <form action="{{ route('participants.update', $participant->id) }}" method="POST">
            ...
            <div class="mb-6">
                ...
                <textarea name="address" id="address"
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('address') border-pink-500 @enderror">{{ old('address', $participant->address) }}</textarea>
                @error('address')
                    <p class="text-pink-500 text-xs italic">{{ $message }}</p>
                @enderror
            </div>
